...
class CarlaRosBridge(object):
    ...
    def _update_actors(self, current_actors):
        ...
        # Problem: "terminate called after throwing an instance of 'std::bad_weak_ptr'"
        do_not_delete_list = rospy.get_param('do_not_delete_from_bridge_actors')
        ...
        if deleted_actors:
            for id_to_delete in deleted_actors:
                ...
                with self.update_lock:
                    ...
                    # Problem: "terminate called after throwing an instance of 'std::bad_weak_ptr'"
                    # Solution: doble delete of the actor is the reason. Any object created by us is double deleted due to bridge is also try to delete.
                    # it is solved by not deleteing the object already created by us
                    proceed_it = True
                    if actor.__class__.__name__ in do_not_delete_list:
                        proceed_it = False
                    if proceed_it:
                ...
                for pseudo_actor in self.pseudo_actors:
                    if pseudo_actor.get_parent_id() == id_to_delete:
                        ...
                        # it is solved by not deleteing the object already created by us
                        proceed_it = True
                        if pseudo_actor.__class__.__name__ in do_not_delete_list:
                            proceed_it = False
                        if proceed_it:
