...
for scenario_data_file in evaluation['scenario_data_per_episode']:
    ...
    for episode_item in scenario_data_file:
        ...
        for item in episode_item['timestep_failure']:
            if item == 1:
                rss_failure += 1
        ratio = round((rss_failure/len(episode_item['timestep_failure']))*100, 2)
...
if plot_main:
    ...
    rss_failure = 0
    for item in failure_scenario['timestep_failure']:
        if item is 1:
            rss_failure += 1    

    failure_timestep = rss_failure
    success_timestep = len(failure_scenario['timestep_failure']) - failure_timestep
    ...
    rss_failure = 0
    for item in success_scenario['timestep_failure']:
        if item is 1:
            rss_failure += 1

    failure_timestep = rss_failure
    success_timestep = len(success_scenario['timestep_failure']) - failure_timestep
    ...
    ts = []
    for i in range(0, int(failure_scenario['timesteps'])):
        ts.append(i)
    x = ts; y1 = failure_scenario['speed_ped']; y2 = failure_scenario['speed_ego'] 
    ...
    ts = []
    for i in range(0, len(failure_scenario['timestep_failure'])):
        ts.append(i)
    ...
    plt.ylabel("Failure")
    plt.plot(ts, failure_scenario['timestep_failure'], linewidth=2) 
    plt.savefig(path_to_save_dir+"challenging_scenario_timesteps")
    plt.close()


    ts = []
    for i in range(0, int(success_scenario['timesteps'])):
        ts.append(i)
    x = ts; y1 = success_scenario['speed_ped']; y2 = success_scenario['speed_ego'] 
    plt.figure(figsize=(6,1), dpi=120) # 10 is width, 4 is height
    plt.rcParams.update({'font.size': 22})
    # Plot Line1 (Left Y Axis)
    fig, ax1 = plt.subplots(1,1,figsize=(16,7), dpi= 80)
    ...
    ts = []
    for i in range(0, len(success_scenario['timestep_failure'])):
        ts.append(i)
    ...
    plt.xlabel("Time steps")
    plt.ylabel("Failure")
    plt.plot(ts, success_scenario['timestep_failure'], linewidth=2) 
