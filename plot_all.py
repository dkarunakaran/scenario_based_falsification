...
for scenario_data_file in evaluation['scenario_data_per_episode']:
    ...
    for episode_item in scenario_data_file:
        ...
        if ratio < ratio_per_limit and reward <= 0.25:
...
for scenario_data_file in evaluation['scenario_data_per_episode']:
    ...
    for episode_item in scenario_data_file:
        ...
        ratio = round((rss_failure/modified_timestep)*100, 2)
        reward = evaluation['reward_data_per_episode'][file_index][file_episode_no]
        if ratio >= ratio_per_limit or reward >= 0.25:
...
if plot_main:
    ...
    plt.xlabel("Risky timesteps percentage")
    plt.savefig(name)
    plt.close()
    ...
    for timestep in range(0, modifed_failure_timestep):
        ...
        distance.append(failure_scenario['euclidean_distance'][timestep])
    x = ts; y1 = speed_ped; y2 = speed_ego; y3 = distance
    ...
    ax1.legend(bbox_to_anchor=(0., .985, 1., .1), ncol=2, mode="expand", borderaxespad=0., prop={"size":18})
    
    # Plot Line2 (Right Y Axis)
    ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis
    ax2.plot(x, y3, color='tab:blue', label= "Euclidean distance", linewidth=3)
    ax2.legend(bbox_to_anchor=(0, .92, 1., .1), prop={"size":18})
    

    # Just Decorations!! -------------------
    # ax1 (left y axis)
    ax1.set_xlabel('Timesteps', fontsize=20)
    ax1.set_ylabel('Speed(m/s)', fontsize=20)
    ax1.tick_params(axis='y', rotation=0)

    
    # ax2 (right Y axis)
    ax2.set_ylabel("Distace(m)", color='tab:blue', fontsize=20)
    ...
    #-------------------------------------------------------------------
    # Comparing failure timesteps and success timesteps

    rss_failure = 0
    for timestep in range(0, modifed_failure_timestep):
        if failure_scenario['timestep_failure'][timestep] is 1:
            rss_failure += 1
    failure_timestep_fs = rss_failure
    success_timestep_fs = modifed_failure_timestep - failure_timestep_fs

    rss_failure = 0
    for timestep in range(0, modified_success_timestep):
        if success_scenario['timestep_failure'][timestep] is 1:
            rss_failure += 1
    failure_timestep_ss = rss_failure
    success_timestep_ss = modified_success_timestep - failure_timestep_ss


    # data to plot
    n_groups = 2
    means_failure = (failure_timestep_fs, failure_timestep_ss)
    means_success = (success_timestep_fs, success_timestep_ss)
    ...
    rects1 = plt.bar(index, means_failure, bar_width,
    alpha=opacity,
    color='b',
    label='High-risk ts')
    ...
    plt.ylabel('No. of timesteps(ts)')
    plt.xticks(index + bar_width, ('Challenging scenario', 'Non-challenging\n scenario'))
    plt.legend()
    plt.rcParams.update({'font.size': 12})
    plt.tight_layout()
    plt.savefig(path_to_save_dir+"bar_chart")
    plt.close()
